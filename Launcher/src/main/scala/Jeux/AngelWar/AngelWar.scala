...
object AngelWar extends Game{
	...
	def game_starter () = {
		AngelWar.nb_of_moves = 0
		AngelWar.maj_nb_of_moves(0)
		//game_frame_content.bottom_panel.background = AWGE.bottom_panel_color_list(AWGE.no_color_mode)
		...
		AngelWar.initial_nb_of_white_square = AngelWar.nb_of_white_square
		AngelWar.nb_of_moves = 0	//Car les flips initiaux ne comptent pas dans les flips effectués par le joueur
		AngelWar.maj_nb_of_moves(0)
		AngelWar.initial_board = AngelWar.board
	...
	def game_action_restart() : Unit = {
		AngelWar.nb_of_moves = 0
		AngelWar.maj_nb_of_moves(0)
		AngelWar.nb_of_white_square = initial_nb_of_white_square
		AngelWar.board = AngelWar.initial_board
		//Initialise chaque label selon le board
		for (y <- 0 until nb_of_rows) {
			for (x <- 0 until nb_of_cols) {
				val case_colour = board(x)(y)._1
